...
namespace Armaz::GIC {
	void setGICAddress(void *dist, void *rdist);
	uint32_t initGIC();
	bool enableGIC();
	uint32_t getAffinity();
	uint32_t getRedistID(uint32_t affinity);
	bool wakeUpRedist(uint32_t rd);
	void setPriorityMask(uint32_t);
	void enableGroup0Ints();
	void enableGroup1Ints();

	extern volatile uint32_t *GICD_CTLR;
	extern volatile uint32_t *GICD_ISENABLER;
	extern volatile uint32_t *GICC_CTLR;
	extern volatile uint32_t *GICC_PMR;
	extern volatile uint32_t *GICC_BPR;
	...
	struct RdistIf {
		RdistLpisIf lpis __attribute__((aligned(0x10000)));
		RdistSgisIf sgis __attribute__((aligned(0x10000)));

#ifdef GICV4
		RdistVlpisIf vlpis __attribute__((aligned(0x10000)));
		RdistResIf   res   __attribute__((aligned(0x10000)));
#endif
	};
	...
	extern volatile DistIf *gicDist;
}
